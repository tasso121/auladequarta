...
from loo1.comando.While import While
...
from loo1.expressao.binaria.ExpOr import ExpOr
from loo1.expressao.leftExpression.AcessoAtributoId import AcessoAtributoId
...
def main():
        ...
        DecProcedimentoComposta(
            ...
            DecProcedimentoComposta(
                DecProcedimentoSimples(
                    ...
                    ListaDeclaracaoParametro(DecParametro(Id("v"), TipoPrimitivo.TIPO_INTEIRO)),
                    ComDeclaracao(
                        SimplesDecVariavel(TipoClasse(Id("LValor")), Id("aux"), This()),
                        Sequencial(
                            While(
                                ExpNot(
                                    ExpOr(
                                        ExpEquals(AcessoAtributoId(Id("aux"), Id("prox")), ValorNull()),
                                        ExpEquals(
                                            AcessoAtributoId(AcessoAtributoId(Id("aux"), Id("prox")), Id("valor")),
                                            Id("v"),
                                        ),
                                    )
                                ),
                                Atribuicao(Id("aux"), AcessoAtributoId(Id("aux"), Id("prox"))),
                            ),
                            IfThenElse(
                                ExpNot(ExpEquals(AcessoAtributoId(Id("aux"), Id("prox")), ValorNull())),
                                Atribuicao(
                                    AcessoAtributoId(Id("aux"), Id("prox")),
                                    AcessoAtributoId(AcessoAtributoId(Id("aux"), Id("prox")), Id("prox")),
                                ),
                                Skip(),
                            ),
                        ),
    ...
    corpoComando = ComDeclaracao(
        ...
        Sequencial(
            ChamadaMetodo(Id("lv"), Id("insere"), ListaExpressao(ValorInteiro(2))),
            Sequencial(
                ...
                Sequencial(
                    ...
                    Sequencial(
                        ...
                        Sequencial(
                            ChamadaMetodo(Id("lv"), Id("remove"), ListaExpressao(ValorInteiro(3))),
                            ChamadaMetodo(Id("lv"), Id("print"), ListaExpressao()),
                        ),
                    ),
                ),
